...
function create_critic_model(visual::Bool, rng::AbstractRNG, state_size::Int, action_size::Int)
    init=glorot_uniform(rng)
    if visual
        Chain(
...
create_critic(visual::Bool, rng::AbstractRNG, state_size::Int, action_size::Int) = TwinDelayedDDPGCritic{visual}([create_critic_model(visual, rng, state_size, action_size), create_critic_model(visual, rng, state_size, action_size)])