...
function tensorboard_groundtruth_hook(
    agent,
    tf_log_dir="logs/Lift";
    save_checkpoints=true,
    save_frequency=10_000,
    agent_name="agents/groundtruth/",
)
    ...
    return hook = ComposedHook(
        ...
        DoEveryNStep() do t, agent, env
            with_logger(lg) do
                @info "losses" critic_loss = agent.policy.critic_loss actor_loss =
                    agent.policy.actor_loss
            end
...
agent = Agent(;
    policy=TwinDelayedDDPGBasePolicy(;
        behavior_actor=NeuralNetworkApproximator(;
            model=create_actor(), optimizer=ADAM(1e-4)
        ),
        behavior_critic=NeuralNetworkApproximator(;
            model=create_critic(), optimizer=ADAM(1e-4)
        ),
    ...
    trajectory=CircularArraySARTTrajectory(;
        capacity=3_000_000, state=Vector{Float32} => (ns,), action=Float32 => (na,)
    ),
)

stop_condition = StopAfterStep(200_000; is_show_progress=!haskey(ENV, "CI"));

hook = tensorboard_groundtruth_hook(
    agent,
    string("logs/", env_name);
    save_frequency=100_000,
    agent_name=string("agents/groundtruth/", env_name),
)
