...
function retrieve_logged_data(log_name::String, index::Int)
    ...
    return t, y = get(hist, Symbol("reward/total_reward_per_episode.rewards[end]"))
end
...
function run_averaging_internal(; directory, pattern)
    ...
    return average_run = mean(values_run_common; dims=2)
end

function run_averaging(;
    directory="newer_logs/", environment::String, select_baseline::Bool
)
    if select_baseline
    ...
    return run_averaging_internal(; directory=directory, pattern=pattern)
end
